<template>
  <div class="flex justify-center items-center h-screen w-auto">
    ...
    <UForm :state="state" class="h-auto w-[500px] flex flex-col gap-5 rounded p-8 bg-custom-50 dark:bg-custom-900 shadow-lg">
      ...
      <UButton @click="clientLogin" :label="labelClient" :loading-icon="loadIcon" :loading="loading" class="flex justify-center items-center gap-1 py-2 rounded dark:text-custom-50"/>

      <Divider />

      <UButton to="/login/admin" label="Continue as Admin" icon="i-lucide-square-user-round" class="flex justify-center items-center gap-1 py-2 rounded bg-custom-700 hover:bg-custom-800 dark:bg-custom-600 dark:hover:bg-custom-700 dark:text-custom-50"/>
